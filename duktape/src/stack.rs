...
pub trait Stack {
    ...
    fn pop_string(&mut self) -> Result<crate::Value, Error> {
        ...
        unsafe {
            duk_pop(self.ptr_mut());
        }
        Ok(crate::Value::String(result))
    }

    fn pop(&mut self) -> Result<crate::Value, Error> {
        let typeid: u32 = unsafe { duk_get_type(self.ptr_mut(), -1) as u32 };
        match typeid {
            ...
            DUK_TYPE_BOOLEAN => {
                let r = Ok(crate::Value::from(
                    unsafe { duk_get_boolean(self.ptr_mut(), -1) } != 0,
                ));
                unsafe {
                    duk_pop(self.ptr_mut());
                }
    ...
    fn pop_ptr<T>(&mut self) -> Result<*mut T, Error> {
        ...
        match typeid {
            DUK_TYPE_BUFFER => unsafe {
                Ok(duk_get_buffer_data(self.ptr_mut(), -1, std::ptr::null_mut()) as *mut T)
            },
            DUK_TYPE_POINTER => unsafe {
                let ptr = duk_get_pointer(self.ptr_mut(), -1) as *mut T;
                Ok(ptr)
            },
            _ => Err(Error::NoneType),
        }
    ...
    fn put_global_pointer<T: Sized>(&mut self, name: &str, ptr: *mut T) -> Result<(), Error> {
        ...
        unsafe {
            duk_push_pointer(self.ptr_mut(), ptr as *mut c_void);
            duk_put_global_string(self.ptr_mut(), name.as_ptr());
        }
        Ok(())
    }
    ...
    fn push_global_stash(&mut self) {
        unsafe {
            duk_push_global_stash(self.ptr_mut());
        }
    }

    fn dup(&mut self, from_idx: i32) {
        unsafe {
            duk_dup(self.ptr_mut(), from_idx);
        }
    }

    fn drop(&mut self) {
        unsafe { duk_pop(self.ptr_mut()) }
    }
    ...
    fn swap_top(&mut self, idx: i32) {
        unsafe {
            duk_swap_top(self.ptr_mut(), idx);
        }
    }

    fn get_prop(&mut self, obj_idx: i32) {
        unsafe {
            duk_get_prop(self.ptr_mut(), obj_idx);
        }
    }

    fn put_prop(&mut self, obj_idx: i32) {
        unsafe {
            duk_put_prop(self.ptr_mut(), obj_idx);
        }
    }
    ...
    fn get_global_string(&mut self, name: &str) -> Result<(), Error> {
        ...
        let exists = unsafe { duk_get_global_string(self.ptr_mut(), cstr.as_ptr()) } != 0;
        if exists {
            Ok(())
        } else {
            Err(Error::NoneType)
    ...
    fn get_string(&mut self, idx: i32) -> Result<String, Error> {
        let cstr = unsafe { duk_get_string(self.ptr_mut(), idx) };
    ...
    fn require_stack(&mut self, sz: i32) {
        unsafe {
            duk_require_stack(self.ptr_mut(), sz);
        }
    }

    fn push_null(&mut self) {
        unsafe {
            duk_push_null(self.ptr_mut());
        }
    }
    ...
    fn push_string(&mut self, name: &str) -> Result<(), Error> {
        let cstr = CString::new(name).map_err(|_| Error::BadString)?;
        unsafe {
            duk_push_string(self.ptr_mut(), cstr.as_ptr());
        }
        Ok(())
    }
